...
#define LOG_D(fmt, ...) MS_LOGD(MS_DEBUG_LOG_NORMAL, fmt, ##__VA_ARGS__)
#else
#define  LOG_D(...)  ((void)0)
#endif*/
...
//#define LOG_D(fmt, ...) __android_log_write(ANDROID_LOG_ERROR, "SENSOR4", fmt...)
#define LOG_D(fmt, ...) __android_log_print(ANDROID_LOG_ERROR, "SENSOR4", fmt, ##__VA_ARGS__)
//#define  LOG_D(...)  ((void)0)
...
static struct sensor_t *get_sensor_by_handle(int handle) {
    int index = 0;

    for (index = 0; index < MAX_NUM_SENSORS; index++) {
        if (sSensorList[index].handle == handle) {
            return &sSensorList[index];
        }
    }
...
{
    sensors_poll_context	*poll_ctx = (void *)dev;
    struct sensor_t *pSensor = NULL;
    int i = 0;
    int nSensorStatus = 0;
    char acTem[PROPERTY_VALUE_MAX] = {0};
    char value[PROPERTY_VALUE_MAX] = {0};

    LOG_D("sensor__activate handle: %d, enabled: %d", handle, enabled);
//	__android_log_write(ANDROID_LOG_ERROR, "SENSOR4", "xcy --- sensor__activate");
    if ((pSensor = get_sensor_by_handle(handle)) == NULL) {
        LOG_D("sensor__activate pSensor==NULL");
        return -1;
    }
    ...
    /*hw.vsensor.enable 属性值是32位，sensor type 对应其位数，位数从0开始技，
       如:sensor type 为1， 则hw.vsensor.enable 属性值的第二位对应该sensor type.
       每个sensor type 有open  (1) 与close (0) 两种状态 */
    if (pSensor->type >= 32)
    {
        LOG_D("sensor__activate pSensor->type >= 32,  %d", pSensor->type);
        return -1;
    }

    if (property_get("hw.vsensor.enable", value, "0") > 0)
    {
        nSensorStatus = atoi(value);
    }
    else
    {
	LOG_D("error property_get hw.vsensor.enable fail,use default 0");
    }

    LOG_D("sensor__activate nSensorStatus:%d, pSensor->type:%d, enabled:%d", 
		nSensorStatus, pSensor->type, enabled);

    if (enabled)
    {	
         //将open 状态设置到pSensor->type 对应的位数中
        nSensorStatus |= (1 << pSensor->type);
    }
    else
    {
        //将close 状态设置到pSensor->type 对应的位数中
        nSensorStatus &= (~(1 << pSensor->type));
    }

    snprintf(acTem, PROPERTY_VALUE_MAX, "%d", nSensorStatus);
	
    if (property_set("hw.vsensor.enable", acTem) < 0)
    {
        LOG_D("property_set hw.vsensor.enable:%s fail!", acTem);

        for (i = 0; i < 10; i++)
        {
            if ( (property_get("hw.vsensor.enable", value, NULL) >0) && (strcmp(acTem, value) == 0))
            {
                break;
            }
    
            usleep(10 * 1000);		
            property_set("hw.vsensor.enable", acTem);
			
    	    LOG_D("property_set hw.vsensor.enable:%s, reset count:%d", acTem, i+1);
        }
    }

    LOG_D("sensor__activate :nSensorStatus:%d", nSensorStatus);
...
//	__android_log_write(ANDROID_LOG_ERROR, "SENSOR4", "xcy --- pick_sensor");
...
static int sensor__poll(struct sensors_poll_device_t *dev, sensors_event_t *data, int count)
{
 ...
 //   __android_log_write(ANDROID_LOG_ERROR, "SENSOR4", "xcy --- entry sensor__poll");
 ...
 //       LOG_D("sensor__poll len: %d", len);
        ...
        */
...

        {
            ...
            if (strstr(pcDataTmp, "datatype=") == NULL) continue;
            str1 = pcDataTmp + strlen("datatype=");
            if (str1 - pcDataTmp >= nDataLen) continue;
            str2 = strstr(str1, "&sensortype=");
            if (str2 == NULL || str2 - str1 >= sizeof(dataTypeStr)) continue;
            ...
            if (str2 == NULL || str2 - str1 >= sizeof(sensorTypeStr)) continue;
            ...
            if (sensorType == SENSOR_TYPE_ACCELERATION || sensorType == SENSOR_TYPE_GYROSCOPE
                ...
                if (str1 - pcDataTmp >= nDataLen) continue;
                str2 = strstr(str1, "&x=");
                if (str2 == NULL || str2 - str1 >= sizeof(accuracyStr)) continue;
                ...
                if (str1 - pcDataTmp >= nDataLen) continue;
                str2 = strstr(str1, "&y=");
                if (str2 == NULL || str2 - str1 >= sizeof(xStr)) continue;
                ...
                if (str1 - pcDataTmp >= nDataLen) continue;
                str2 = strstr(str1, "&z=");
                if (str2 == NULL || str2 - str1 >= sizeof(yStr)) continue;
                ...
                if ((str2 = strstr(str1, "&value=")) == NULL) {
                    strncpy((void *)zStr, str1, sizeof(zStr) - 1);
                    z = atof(zStr);
                    //LOG_D("z = %f\n", z);
                } else if (str2 - str1 < sizeof(zStr)) {
                    ...
                    strncpy((void *)valueStr, str1, sizeof(valueStr) - 1);
                    value = atof(valueStr);
                    //LOG_D("value = %f\n", value);
                ...
                if (str2 == NULL || str2 - str1 >= sizeof(azimuthStr)) continue;
                ...
                if (str2 == NULL || str2 - str1 >= sizeof(pitchStr)) continue;
                ...
                strncpy((void *)rollStr, str1, sizeof(rollStr) - 1);
                roll = atof(rollStr);
                //LOG_D("roll = %f\n", roll);
        ...
        poll_ctx->sensors[poll_ctx->pendingSensors].type = sensors_type;
...
//	__android_log_write(ANDROID_LOG_ERROR, "SENSOR4", "xcy --- open_sensors");
    LOG_D("open_sensors name: %s", name);
	
//	poll[sizeof(sensors_poll_context) + 100] = 0;
//	*((int *)0) = 1;
