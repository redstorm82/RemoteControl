...
import example.com.remotecontrol.mcontrol.inputcontrol.KeyboardCommand;
...
public class RemoteKeyboard {
    ...
    public RemoteKeyboard(String ip) {
        ...
        udpClient = new MbusUdp(ip, Constants.REMOTE_PORT);
        try {
            udpClient.connect();
    ...
    public void remoteSendDownAndUpKeyCode(int keycode) {
        ...
        try {
            if (null == udpClient) {
                udpClient = new MbusUdp(mIP, Constants.REMOTE_PORT);
                udpClient.connect();
            }
